...
const Card = ({data:{category, title, location, price,_id}}) => {
    ...
    return (
        <div className=" w-[250px] border-2 border-[#304ffe58] p-[10px] m-[10px] rounded-[10px]">
            ...
            <Link href={`/buy-residential/${_id}`} className=" flex items-center justify-between mt-5 text-[0.95rem] font-normal text-[#304ffe]">مشاهده آگهی <BiLeftArrowAlt className=" text-2xl"/></Link>
        </div>
    );
}
