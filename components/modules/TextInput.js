...
const TextInput = ({title,name,profileData, setProfileData, textarea=false}) => {
    ...
    return (
        <div>
            ...
            :
            <input name={name} value={profileData[name]} onChange={changeHandler} className=" mb-10 w-[300px] border border-[#304ffe] border-dashed text-gray-600 rounded p-[10px] text-base h-10 outline-none focus:outline-none focus:border-solid"/>
            }
        </div>
    );
}
